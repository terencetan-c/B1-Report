function [flag,time,i] = task5_branch_and_bound(W,b,xmin,xmax,P_max)
    ...
    [~,ymax] = linear_programming_bound2(W,b,transpose(X_j(:,1)),transpose(X_j(:,2)));    % Compute upper bound
    ...
    refined_X = projected_gradient_ascent(W,b,input,xmin,xmax);
    ymin = max(compute_nn_outputs(W,b,refined_X));
    ...
    for i = 1:P_max
        ...
        [~,new_upperbound1] = linear_programming_bound2(W,b,transpose(X1_prime(:,1)),transpose(X1_prime(:,2)));
        [~,new_upperbound2] = linear_programming_bound2(W,b,transpose(X2_prime(:,1)),transpose(X2_prime(:,2)));
        ...
        refined_X1 = projected_gradient_ascent(W,b,input1,X1_prime(:,1),X1_prime(:,2));
        new_lowerbound1 =  max(compute_nn_outputs(W,b,refined_X1)); % Lower bound for first new subdomain
        
        input2 = generate_inputs(transpose(X2_prime(:,1)),transpose(X2_prime(:,2)), k_max);  % Generate the inputs
        input2 = transpose(input2);
        refined_X2 = projected_gradient_ascent(W,b,input2,X2_prime(:,1),X2_prime(:,2));
        new_lowerbound2 = max(compute_nn_outputs(W,b,refined_X2));  % Lower bound for second new subdomain
        ...
        P{J} = X1_prime;    % Insert first new subdomain into Partition set
        P{i+1} = X2_prime;  % Insert second new subdomain into Partition set
        
        %P{end+1} = X1_prime;    
        %P{end+1} = X2_prime;    
        
        %bounds(end+1,:) = [new_lowerbound1,new_upperbound1];    
        %bounds(end+1,:) = [new_lowerbound2,new_upperbound2];    
        
        bounds(J,:) = [new_lowerbound1,new_upperbound1];    % Insert bounds for X1_prime into Bounds set
        bounds(i+1,:) = [new_lowerbound2,new_upperbound2];  % Insert bounds for X2_prime into Bounds set
        ...
        % i+k iterations is T(i) + t(i+1) + t(i+2) + .... + t(i+k) where
